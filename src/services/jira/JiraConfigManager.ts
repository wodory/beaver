import { JiraAdapterConfig } from './IJiraAdapter.js';
...
export class JiraConfigManager {
   ...
   */
  loadFromFile(filePath: string): JiraAdapterConfig {
    try {
      const configPath = path.resolve(process.cwd(), filePath);
      const configData = fs.readFileSync(configPath, 'utf8');
      const config = JSON.parse(configData) as JiraAdapterConfig;
   ...
   * 설정이 유효한지 확인합니다.
   */
  validateConfig(config: JiraAdapterConfig): boolean {
    if (!config.baseUrl || !config.username || !config.apiToken) {
      return false;
    }
