...
import { ChangeFailureRateDataPoint } from '@/store/dashboardStore';

/**
 * 변경 실패율 추이 차트 컴포넌트
 * 선택된 기간의 일별 변경 실패율을 복합 차트로 시각화합니다.
 */
interface ChangeFailureRateChartProps {
  multiRepoMode?: boolean;
  multiRepoData?: { [repo: string]: ChangeFailureRateDataPoint[] };
  colorPalette?: string[];
}

const ChangeFailureRateChart: React.FC<ChangeFailureRateChartProps> = ({ 
  multiRepoMode = false, 
  multiRepoData = {},
  colorPalette = ['#007AFF', '#FF2D55', '#5AC8FA', '#FF9500', '#4CD964'] 
}) => {
  const { changeFailureRateData } = useStore();

  // 디버그 로깅 추가
  useMemo(() => {
    if (multiRepoMode) {
      console.log('ChangeFailureRateChart - 다중 저장소 모드 데이터:', {
  ...
  const repoAverages = useMemo(() => {
    ...
    Object.entries(multiRepoData).forEach(([repo, dataPoints]) => {
      ...
      const sum = dataPoints.reduce((acc, item) => acc + ((item.rate || 0) * 100), 0);
      const avg = sum / dataPoints.length;
      result[repoName] = Number(avg.toFixed(1)); // 최소값 설정 제거
    });
    
    console.log('ChangeFailureRateChart - 계산된 평균값:', result);
    return result;
  }, [multiRepoMode, multiRepoData]);
  ...
  if (multiRepoMode) {
    ...
    return (
      <div className="h-full">
        <p className="text-base font-medium mb-4 text-center">평균 변경 실패율</p>
  ...
  // 단일 저장소 모드 (기존 코드)
  if (!changeFailureRateData || changeFailureRateData.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
  ...
  // 평균값 계산 (퍼센트로 변환)
  const average = (changeFailureRateData.reduce((sum, item) => sum + item.rate, 0) / changeFailureRateData.length) * 100;
  ...
  return (
    <div className="h-full">
      ...
      <ResponsiveContainer width="100%" height="90%">
        <BarChart
          data={[{ name: '변경 실패율', value: Number(average.toFixed(1)) }]}
