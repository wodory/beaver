...
import { DeploymentFrequencyDataPoint } from '@/store/dashboardStore';

/**
 * 배포 빈도 차트 컴포넌트
 * 선택된 기간의 일별 배포 횟수를 복합 차트로 시각화합니다.
 */
interface DeploymentFrequencyChartProps {
  multiRepoMode?: boolean;
  multiRepoData?: { [repo: string]: DeploymentFrequencyDataPoint[] };
  colorPalette?: string[];
}
...
const DeploymentFrequencyChart: React.FC<DeploymentFrequencyChartProps> = ({ 
  ...
  const { deploymentFrequencyData } = useStore();

  // 디버그 로깅 추가
  useMemo(() => {
    if (multiRepoMode) {
      console.log('DeploymentFrequencyChart - 다중 저장소 모드 데이터:', {
  ...
  const repoAverages = useMemo(() => {
    ...
    Object.entries(multiRepoData).forEach(([repo, dataPoints]) => {
      ...
      const sum = dataPoints.reduce((acc, item) => acc + (item.count || 0), 0);
      // 평균 계산 시 시간당 배포 횟수로 변환 (일별 데이터를 24로 나눔)
      const avg = (sum / dataPoints.length) / 24;
      result[repoName] = Number(avg.toFixed(1)); // 최소값 설정 제거
    });
    
    console.log('DeploymentFrequencyChart - 계산된 평균값:', result);
    return result;
  }, [multiRepoMode, multiRepoData]);
  ...
  if (multiRepoMode) {
    ...
    return (
      <div className="h-full">
        <p className="text-base font-medium mb-4 text-center">시간당 평균 배포 횟수</p>
        <ResponsiveContainer width="100%" height="90%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(value: number) => [`${value} 회/시간`, '배포 빈도']}
            />
  ...
  // 단일 저장소 모드 (기존 코드)
  if (!deploymentFrequencyData || deploymentFrequencyData.length === 0) {
    return (
      <div className="flex items-center justify-center h-full">
  ...
  // 평균값 계산
  const totalDeployments = deploymentFrequencyData.reduce((sum, item) => sum + item.count, 0);
  // 배포 빈도를 시간당으로 계산 (일별 데이터를 24로 나눔)
  const hourlyAverage = (totalDeployments / deploymentFrequencyData.length) / 24;
  ...
  return (
    <div className="h-full">
      ...
      <ResponsiveContainer width="100%" height="90%">
        <BarChart
          data={[{ name: '시간당 배포 횟수', value: Number(hourlyAverage.toFixed(1)) }]}
