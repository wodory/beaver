...
import { MTTRDataPoint } from '@/store/dashboardStore';

interface MeanTimeToRestoreChartProps {
  multiRepoMode?: boolean;
  multiRepoData?: { [repo: string]: MTTRDataPoint[] };
  colorPalette?: string[];
}

const MeanTimeToRestoreChart: React.FC<MeanTimeToRestoreChartProps> = ({ 
  multiRepoMode = false, 
  multiRepoData = {},
  colorPalette = ['#007AFF', '#FF2D55', '#5AC8FA', '#FF9500', '#4CD964'] 
}) => {
  const { mttrData } = useStore();

  // 디버그 로깅 추가
  useMemo(() => {
    if (multiRepoMode) {
      console.log('MeanTimeToRestoreChart - 다중 저장소 모드 데이터:', {
  ...
  const repoAverages = useMemo(() => {
    ...
    Object.entries(multiRepoData).forEach(([repo, dataPoints]) => {
      ...
      const sum = dataPoints.reduce((acc, item) => acc + (item.mttr || 0), 0);
      const avg = sum / dataPoints.length;
      result[repoName] = Number(avg.toFixed(1)); // 최소값 설정 제거
    });
    
    console.log('MeanTimeToRestoreChart - 계산된 평균값:', result);
    return result;
  }, [multiRepoMode, multiRepoData]);
  ...
  if (multiRepoMode) {
    ...
    return (
      <div className="h-full">
        <p className="text-base font-medium mb-4 text-center">평균 복구 시간</p>
        <ResponsiveContainer width="100%" height="90%">
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(value: number) => [`${value} 시간`, '평균 복구 시간']}
            />
  ...
  // 평균값 계산
  const average = mttrData.reduce((sum, item) => sum + item.mttr, 0) / mttrData.length;
  ...
  return (
    <div className="h-full">
      ...
      <ResponsiveContainer width="100%" height="90%">
        <BarChart
          data={[{ name: '평균 복구 시간', value: Number(average.toFixed(1)) }]}
...
export default MeanTimeToRestoreChart; 