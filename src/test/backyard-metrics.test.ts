...
describe('Wodory Backyard 저장소 메트릭스 테스트', () => {
  it('wodory/backyard 저장소의 전체 기간 지표를 계산합니다', async () => {
    // Wodory Backyard 저장소 정보
    const owner = 'wodory';
    const repo = 'backyard';
    
    // 기간 설정: 전체 기간 (since 파라미터를 전달하지 않음)
    const until = new Date().toISOString();
    
    console.log(`${owner}/${repo} 저장소의 전체 기간 메트릭스 조회 중...`);
    
    try {
      // PR 데이터 가져오기 (since 파라미터 없이 호출)
      const pullRequests = await fetchPullRequests(owner, repo, '', until);
