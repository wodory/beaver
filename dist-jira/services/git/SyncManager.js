...
export class SyncManager {
    ...
    async getRepository(repoId) {
        ...
        const repos = await dbAdapter.select(dbAdapter.db.select()
     ...
     * @returns 동기화 결과
     */
    async syncRepository(repoId, forceFull = false, syncJira = true) {
        ...
        try {
            ...
            // 마지막 동기화 시간 업데이트
            await dbAdapter.update(schema.repositories, { lastSyncAt: new Date() }, eq(schema.repositories.id, repoId));
            result.success = true;
     ...
     * @returns 동기화 결과 목록
     */
    async syncAllRepositories(forceFull = false, syncJira = true) {
     ...
     * @returns JIRA 이슈 목록
     */
    async collectJiraIssuesForRepository(repoInfo, since) {
     ...
     * @returns 저장된 이슈 수
     */
    async storeJiraIssues(repoInfo, issues) {
        if (!dbAdapter.db) {
            throw new Error('데이터베이스가 초기화되지 않았습니다.');
        }
        if (issues.length === 0) {
        ...
        try {
            ...
            for (const issue of issues) {
                try {
                    ...
                    const existingIssues = await dbAdapter.select(dbAdapter.db.select()
                    ...
                    if (existingIssues.length > 0) {
                        // 이슈가 이미 존재하면 업데이트
                        await dbAdapter.update(schema.jiraIssues, issueData, eq(schema.jiraIssues.key, issue.key));
                    }
                    else {
                        // 새 이슈 삽입
     ...
     * @returns 저장소 로컬 경로
     */
    async ensureRepositoryPath(repoInfo) {
     ...
     * @returns 저장된 PR 수
     */
    async storePullRequests(repoInfo, pullRequests) {
        let storedCount = 0;
        try {
          ...
          `, [repoInfo.id, pr.number]);
                    if (existingPRs && existingPRs.length > 0) {
                        // 기존 PR 업데이트
                        await dbAdapter.update(schema.pullRequests, {
                            title: pr.title,
                            body: pr.body || '',
                            state: state,
     ...
     * @returns 저장된 리뷰 수
     */
    async storePullRequestReviews(repoInfo, prNumber, reviews) {
        let storedCount = 0;
        try {
            // PR ID 찾기
     ...
     * @returns 사용자 ID
     */
    async getOrCreateUser(login, githubId, avatarUrl) {
        try {
            // 기존 사용자 찾기
