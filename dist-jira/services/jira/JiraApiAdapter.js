...
export class JiraApiAdapter {
     ...
     * JQL 쿼리를 만듭니다.
     */
    buildJqlQuery(options, isCompleted = false) {
        const jqlParts = [];
        // 프로젝트 키로 필터링
    ...
    async calculateIssueStats(issues) {
        ...
        try {
            // 완료된 이슈만 필터링
            const completedIssues = issues.filter(issue => issue.status === 'Done' && issue.resolutionDate !== null && issue.created !== null);
            // 평균 해결 시간 계산 (일 단위)
...
//# sourceMappingURL=JiraApiAdapter.js.map