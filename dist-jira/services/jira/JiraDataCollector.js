...
export class JiraDataCollector {
     ...
     * @param options - 검색 옵션
     */
    async getCompletedIssues(options) {
        this.checkInitialized();
     ...
     * @param options - 검색 옵션
     */
    async getCreatedIssues(options) {
        this.checkInitialized();
     ...
     */
    async calculateIssueStats(issues) {
        this.checkInitialized();
    ...
    async getProjectSummary(startDate, endDate) {
        ...
        const projectsWithCompletedIssues = result.projects.filter((p) => p.completedIssues > 0);
        if (projectsWithCompletedIssues.length > 0) {
            const totalAvgTime = projectsWithCompletedIssues.reduce((sum, p) => sum + p.averageResolutionTimeInDays, 0);
...
//# sourceMappingURL=JiraDataCollector.js.map